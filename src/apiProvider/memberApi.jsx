...
class MemberApiProvider {
    ...
    async getMembersByEvents(params, id) {
        try {
            const response = await apiClient.get(`/members/by-meeting/${id}`,
    ...
    async updateMember(id, memberData) {
        try {
            ...
            if (response.status === 200) {
                ...
                console.error("Failed to update member:", response.data?.message ?? "Something went wrong");
                return { status: false, data: response.data };
            }
        } catch (error) {
    ...
    async updateMemberStatus(id, memberData) {
        try {
            const response = await apiClient.patch(`/members/status/${id}`, memberData);
            if (response.status === 200 || response.status === 201) {
                return { status: true, data: response.data };
            } else {
                console.error("Failed to update member:", response.data?.message ?? "Something went wrong");
