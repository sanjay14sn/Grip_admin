...
const EventListLayer = () => {
  ...
  const handleEditPayment = (paymentId) => {
    ...
    if (paymentToEdit) {
      ...
      const imageField =
        paymentToEdit.image || paymentToEdit.eventImage || paymentToEdit.images;
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.paymentRequirement === "required") {
      if (!amountStr) {
    ...
    try {
      ...
      submissionData.append(
        "paymentRequired",
        formData.paymentRequirement === "required" ? true : false
      );
            ...
            <tbody>
              {paymentDetails.map((payment, index) => (
                <tr key={payment._id}>
                  ...
                  <td>{payment.amount === 0 ? "-" : `₹${payment.amount}`}</td>
                ...
                <div className="row">
                  ...
                  <div className="col-12 mb-20">
                    <label className="form-label fw-semibold text-primary-light text-sm mb-8">
                      ...
                      Amount{" "}
                      {formData.paymentRequirement !== "optional" && (
                    ...
                    <input
                      ...
                      value={
                        formData.paymentRequirement === "optional"
                          ? "" // Not Required → show empty
                          : formData.amount === 0
                          ? "" // Required but amount=0 → show empty
                          : formData.amount
                      }
                      disabled={formData.paymentRequirement === "optional"} // Not Required → disable
                      onChange={(e) => {
                        if (
